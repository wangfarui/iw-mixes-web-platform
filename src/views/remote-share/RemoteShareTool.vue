    ...
    <main class="remote-share-grid">
      <section class="panel session-panel">
        ...
        <template v-if="!device">
          <el-alert v-if="sessionEnded" type="info" :closable="false" show-icon title="会话已结束" />
          <el-button type="primary" :loading="loading" class="wide-button" @click="createSession">创建新会话</el-button>
          <el-divider>或</el-divider>
          <el-input v-model="joinLink" placeholder="输入 4 位会话码，或粘贴分享链接" clearable />
        ...
        <template v-else>
          ...
          <p v-if="device.joinCode" class="join-code">会话码：<strong>{{ device.joinCode }}</strong><el-button text type="primary" @click="copyText(device.joinCode)">复制</el-button></p>
      ...
      <section class="panel send-panel">
        ...
        <el-tabs v-model="activeTab" stretch>
          <el-tab-pane label="文本" name="text">
            <el-input v-model="textDraft" type="textarea" :rows="10" maxlength="10000" placeholder="输入要共享的文本…" />
          ...
          <el-tab-pane label="图片 / 文件" name="file">
            <el-upload drag :auto-upload="false" :show-file-list="false" :on-change="selectFile">
              ...
              <template #tip><div class="el-upload__tip">单项最大 10 MB</div></template>
            </el-upload>
      ...
      <section class="panel records-panel">
        ...
        <article v-for="record in records" :key="record.id" class="record-item" :class="record.direction">
          ...
          <img v-if="record.imageUrl" :src="record.imageUrl" :alt="record.text" class="record-image">
          <pre>{{ record.text }}</pre>
          <div v-if="record.direction === 'received' || record.file" class="record-actions">
            <el-button v-if="record.direction === 'received' && !record.file && record.fileKind !== 'file'" class="record-action" @click="copyText(record.text)">
              <el-icon><DocumentCopy /></el-icon>复制文本
            </el-button>
            <el-button v-if="record.file" class="record-action record-action-primary" @click="downloadFile(record)">
...
import { beginRemoteShareBinary, claimRemoteShareTexts, closeRemoteShareSession, completeRemoteShareBinary, createRemoteShareSession, downloadRemoteShareChunk, getRemoteShareState, joinRemoteShareSession, joinRemoteShareSessionByCode, pendingRemoteShareBinaries, receiptRemoteShareBinary, RemoteShareRequestError, sendRemoteShareText, uploadRemoteShareChunk, type RemoteShareDevice } from '@/api/remoteShare'
...
interface PendingFile { itemId?: string; chunks?: number; name: string; type: string; kind: 'file' | 'text'; blob?: Blob }
interface RecordItem { id: string; direction: 'sent' | 'received'; text: string; at: string; file?: PendingFile; fileKind?: 'file' | 'text'; imageUrl?: string }
...
const activate = async (secret: string, action: 'create' | 'join') => {
  ...
  try {
    ...
    const nextDevice = action === 'create' ? await createRemoteShareSession(nextMaterial, secret) : await joinRemoteShareSession(nextMaterial)
    await activateMaterial(nextMaterial, nextDevice, secret)
  } catch (error) {
...
const joinSession = async () => {
  const joinCode = joinLink.value.trim()
  if (/^\d{4}$/.test(joinCode)) {
    loading.value = true
    try {
      const joined = await joinRemoteShareSessionByCode(joinCode)
      const nextMaterial = await deriveSessionMaterial(joined.sessionSecret)
      await activateMaterial(nextMaterial, joined, joined.sessionSecret)
    } catch (error) {
      ElMessage.error(error instanceof Error ? error.message : '无法加入会话')
    } finally {
      loading.value = false
    }
    return
  ...
  if (!secret) return ElMessage.warning('请输入 4 位会话码或有效的分享链接')
  await activate(secret, 'join')
}

const activateMaterial = async (nextMaterial: RemoteShareMaterial, nextDevice: RemoteShareDevice, secret: string) => {
...
const refresh = async () => {
  ...
  try {
    ...
    for (const item of binaries) {
      ...
      records.value.unshift({ id: item.itemId, direction: 'received', text: kind === 'text' ? '收到长文本，点击领取后可复制。' : `收到文件：${manifest.name}`, at: new Date().toLocaleTimeString(), file: { itemId: item.itemId, chunks: item.chunks, name: manifest.name, type: manifest.type, kind }, fileKind: kind })
    }
  } catch (error) {
    if (error instanceof RemoteShareRequestError && (error.status === 404 || error.status === 410)) {
      clearSession(true)
      return
    }
    throw error
...
const startPeer = () => {
  ...
  directPeer = new RemoteSharePeer(material.value.roomId, device.value.capability, device.value.slot, async (ciphertext) => {
    ...
    } catch { ElMessage.warning('内容接收失败') }
  }, () => undefined, async (encryptedManifest, encryptedChunks) => {
    try {
      ...
      records.value.unshift({
        ...
        direction: 'received',
        text: `收到文件：${manifest.name}`,
        at: new Date().toLocaleTimeString(),
        file: { name: manifest.name, type: manifest.type, kind: 'file', blob },
        fileKind: 'file'
      })
    } catch { ElMessage.warning('文件接收失败') }
  })
  directPeer.connect()
}
...
const downloadFile = async (record: RecordItem) => {
  ...
  try {
    const pendingFile = record.file
    let blob = pendingFile.blob
    if (!blob) {
      if (!pendingFile.itemId || pendingFile.chunks === undefined) throw new Error('文件已不可用')
      const chunks: Uint8Array[] = []
      for (let index = 0; index < pendingFile.chunks; index += 1) {
        chunks.push(await decryptBytes(material.value.contentKey, await downloadRemoteShareChunk(material.value.roomId, device.value.capability, pendingFile.itemId, index)))
      }
      blob = new Blob(chunks, { type: pendingFile.type })
    }
    if (pendingFile.kind === 'text') {
      record.text = await blob.text()
      await receiptRemoteShareBinary(material.value.roomId, device.value.capability, pendingFile.itemId!)
    ...
    if (pendingFile.itemId) await receiptRemoteShareBinary(material.value.roomId, device.value.capability, pendingFile.itemId)
    record.file = undefined
...
const clearSession = (ended: boolean) => {
  directPeer?.close()
  directPeer = undefined
  window.clearInterval(pollTimer)
  records.value.forEach(record => { if (record.imageUrl) URL.revokeObjectURL(record.imageUrl) })
...
.panel { background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; box-shadow: 0 2px 8px rgb(15 23 42 / 6%); padding: 22px; min-height: 480px; }.panel h2 { margin-top: 0; font-size: 19px; }.wide-button { width: 100%; margin-top: 14px; }.status-copy,.hint { color: #64748b; font-size: 13px; line-height: 1.6; }.join-code { margin: 12px 0; color: #4f46e5; font-size: 16px; letter-spacing: .06em; }.join-code strong { font-size: 24px; margin: 0 6px; }.file-summary { margin: 14px 0; color: #334155; }.upload-icon { font-size: 42px; color: #6366f1; }.record-item { margin-bottom: 12px; padding: 12px; border-radius: 10px; background: #f8fafc; }.record-item.sent { border-left: 3px solid #6366f1; }.record-item.received { border-left: 3px solid #10b981; }.record-meta { color: #94a3b8; font-size: 12px; margin-bottom: 8px; }.record-item pre { white-space: pre-wrap; overflow-wrap: anywhere; font: inherit; margin: 0; }.record-image { display: block; max-width: 100%; max-height: 220px; margin: 8px 0; border-radius: 8px; object-fit: contain; background: #eef2ff; }.record-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; padding-top: 10px; border-top: 1px solid #e5e7eb; }.record-action { height: 30px; margin: 0; padding: 0 12px; border: 1px solid #cbd5e1; border-radius: 16px; color: #475569; background: #fff; font-size: 12px; font-weight: 600; }.record-action:hover { color: #4f46e5; border-color: #a5b4fc; background: #eef2ff; }.record-action-primary { color: #fff; border-color: #4f46e5; background: #4f46e5; }.record-action-primary:hover { color: #fff; border-color: #4338ca; background: #4338ca; }.record-action .el-icon { margin-right: 4px; font-size: 14px; }
