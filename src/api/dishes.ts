...
export const queryDishesPage = (dishesListDto: DishesType.DishesPageDto) => {
    return request.post('/iw-eat/dishes/page', dishesListDto);
}

// 新增菜品记录
export const addDishes = (dishesAddDto: DishesType.DishesAddDto) => {
    return request.post('/iw-eat/dishes/add', dishesAddDto)
}

// 修改菜品记录
export const updateDishes = (dishesUpdateDto: DishesType.DishesUpdateDto) => {
    return request.put('/iw-eat/dishes/update', dishesUpdateDto)
}

// 删除菜品记录
export const deleteDishes = (id: number) => {
    return request.delete('/iw-eat/dishes/delete?id=' + id)
}

// 查询菜品记录详情
export const queryDishesDetail = (id: number) => {
    return request.get('/iw-eat/dishes/detail?id=' + id)
}